...
// ── Shared input classes using CSS variables ──────────────────────────────
const inputCls =
...
const monoTextareaCls =
  "min-h-[420px] w-full rounded-2xl border border-border bg-muted p-4 font-mono text-sm text-foreground focus:border-primary focus:outline-none focus:ring-1 focus:ring-primary transition-colors";
  ...
  return (
    <div className="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
      <Card className="p-6 text-center">
        ...
        <p className="text-3xl font-bold text-foreground">{stats.sermons}</p>
      ...
      <Card className="p-6 text-center">
        ...
        <p className="text-3xl font-bold text-foreground">{stats.testimonies}</p>
...
function SitePagesSection() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground">Editable Site Pages</h2>
      ...
      <Card className="p-6">
        {loading ? (
          ...
          <div className="space-y-4">
            <p className="rounded-2xl border border-secondary/20 bg-secondary/5 p-4 text-sm text-muted-foreground">
            ...
            <textarea
              ...
              spellCheck={false}
              className={`${monoTextareaCls} min-h-[520px]`}
            />
...
function SermonsSection() {
  ...
  const load = async () => {
    try { setLoading(true); setItems((await api.getSermons()) || []); }
    catch (err) { logger.error("Failed to load sermons", err); toast.error("Failed to load sermons"); }
  ...
  const openDialog = (item?: Sermon) => {
    ...
    setForm(item
      ? { title: item.title, scripture: item.scripture, date: item.date, preacher: item.preacher, excerpt: item.excerpt, category: item.category, youtubeUrl: item.youtubeUrl || "", facebookUrl: item.facebookUrl || "" }
      : { title: "", scripture: "", date: "", preacher: "", excerpt: "", category: "", youtubeUrl: "", facebookUrl: "" });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    } catch (err) { logger.error("Failed to save sermon", err); toast.error("Failed to save sermon"); }
    ...
    <CrudSection
      ...
      extraAction={
        <Button
          ...
          size="sm"
        >
          {syncing ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : null}Sync YouTube
        </Button>
      }
      items={items.map((item) => ({
        ...
        onDelete: typeof item.id === "number" ? async () => {
          if (window.confirm("Delete this sermon?")) { await api.deleteSermon(item.id as number); await load(); }
        } : undefined,
      }))}
    >
...
function EventsSection() {
  ...
  const load = async () => {
    try { setLoading(true); setItems((await api.getEvents()) || []); }
    catch (err) { logger.error("Failed to load events", err); toast.error("Failed to load events"); }
    finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  ...
  const openDialog = (item?: EventItem) => {
    ...
    setForm(item
      ? { day: item.day, month: item.month, title: item.title, time: item.time, location: item.location, category: item.category, summary: parsed.summary, programName: parsed.programName, programDate: parsed.programDate, itineraryText: formatItineraryText(parsed.itinerary), featured: Boolean(item.featured) }
      : { day: "", month: "", title: "", time: "", location: "", category: "", summary: "", programName: "", programDate: "", itineraryText: "", featured: false });
    setOpen(true);
  };
  ...
  const save = async () => {
    ...
    const payload = {
      day: form.day, month: form.month, title: form.title, time: form.time,
      location: form.location, category: form.category, featured: form.featured,
    ...
    } catch (err) { logger.error("Failed to save event", err); toast.error("Failed to save event"); }
    finally { setSaving(false); }
  };
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-2">
                <Button type="button" size="sm" variant="outline" onClick={async () => {
                  try { await navigator.clipboard.writeText(getEventShareUrl(editingId)); toast.success("Program link copied"); }
          ...
          <div className="space-y-4">
            ...
            {editingId ? (
              <Card className="border-emerald-500/20 bg-emerald-500/5 p-4">
                ...
                <div className="mt-3">
                  <Button type="button" size="sm" variant="outline" onClick={() => downloadQrSvg("event-qr-dialog", `pcn-event-program-${editingId}.png`)}>Download QR</Button>
                </div>
              </Card>
            ) : null}
            <div className="flex gap-2">
...
function TestimoniesSection() {
  ...
  const load = async () => {
    try { setLoading(true); setItems((await api.getTestimonies()) || []); }
    catch (err) { logger.error("Failed to load testimonies", err); toast.error("Failed to load testimonies"); }
    finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  ...
  const save = async () => {
    ...
    } catch (err) { logger.error("Failed to save testimony", err); toast.error("Failed to save testimony"); }
    finally { setSaving(false); }
  };
...
function GallerySection() {
  ...
  const load = async () => {
    try { setLoading(true); setItems((await api.getGallery()) || []); }
    catch (err) { logger.error("Failed to load gallery", err); toast.error("Failed to load gallery"); }
    finally { setLoading(false); }
  };
  useEffect(() => { void load(); }, []);
  ...
  const save = async () => {
    ...
    try {
      setSaving(true);
      if (editing) { await api.updateGalleryItem(editing.id, formData); toast.success("Gallery item updated"); }
      else { await api.createGalleryItem(formData); toast.success("Gallery item created"); }
      setOpen(false);
      await load();
    } catch (err) { logger.error("Failed to save gallery item", err); toast.error("Failed to save gallery item"); }
    finally { setSaving(false); }
  };
  ...
  return (
    <CrudSection
      ...
      items={items.map((item) => ({
        ...
        onEdit: () => openDialog(item),
        onDelete: async () => { if (window.confirm("Delete this gallery item?")) { await api.deleteGalleryItem(item.id); await load(); } },
      }))}
    >
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
  ...
  return (
    <Card className="p-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          <textarea value={draft} onChange={(e) => setDraft(e.target.value)} spellCheck={false} className={monoTextareaCls} />
          <div className="flex justify-end">
            <Button onClick={save} disabled={saving}>
              {saving ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save Hero Slides
            </Button>
          </div>
        </div>
      )}
...
function ContactSection() {
  ...
  const loadMessages = async () => {
    try { setLoadingMessages(true); setMessages((await api.getContacts()) || []); }
    catch (err) { logger.error("Failed to load contacts", err); toast.error("Failed to load contact messages"); }
  ...
  useEffect(() => { void loadMessages(); void loadContent(); }, []);
    ...
    <div className="space-y-6">
      <Card className="p-6">
        <div className="mb-4 flex items-center gap-2">
          ...
          <h2 className="text-2xl font-bold text-foreground">Contact Inbox</h2>
        </div>
        {loadingMessages ? (
          <div className="flex justify-center py-12"><Loader className="h-8 w-8 animate-spin text-primary" /></div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{message.type}</span>
                      {!message.read ? <span className="rounded-full bg-secondary/10 px-2 py-1 text-xs text-secondary">Unread</span> : null}
                    </div>
                  ...
                  <div className="flex gap-2">
                    {!message.read ? (
                      <Button size="sm" variant="outline" onClick={async () => { await api.markContactRead(message.id); await loadMessages(); }}>Mark Read</Button>
                    ) : null}
                    <Button size="sm" variant="ghost" className="text-destructive" onClick={async () => { if (window.confirm("Delete this message?")) { await api.deleteContact(message.id); await loadMessages(); } }}>
      ...
      <Card className="p-6">
        <div className="mb-4">
          <h2 className="text-2xl font-bold text-foreground">Contact Page Content</h2>
        ...
        {loadingContent ? (
          ...
          <div className="space-y-4">
            <textarea value={contentDraft} onChange={(e) => setContentDraft(e.target.value)} spellCheck={false} className={monoTextareaCls} />
            <div className="flex justify-end">
              <Button onClick={saveContent} disabled={savingContent}>
                {savingContent ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save Contact Content
              </Button>
            </div>
          </div>
        )}
    ...
    <Card className="p-6">
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          ...
          <div className="flex justify-end">
            <Button onClick={save} disabled={saving}>
              {saving ? <Loader className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save Bank Details
            </Button>
          </div>
        </div>
      )}
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          {extraAction}
          <Button size="sm" onClick={onNew}><Plus className="mr-2 h-4 w-4" />New</Button>
        </div>
      </div>
      {loading ? (
        <div className="flex justify-center py-12"><Loader className="h-8 w-8 animate-spin text-primary" /></div>
      ) : items.length === 0 ? (
        <Card className="p-8 text-center"><p className="text-muted-foreground">No records yet</p></Card>
      ) : (
        <div className="grid gap-4">
          {items.map((item) => (
            <Card key={item.id} className="flex items-start justify-between p-4">
              <div className="flex-1">
                <div className="mb-1 flex flex-wrap items-center gap-2">
                  <h3 className="font-semibold text-foreground">{item.title}</h3>
                  {item.badge ? (
                    <span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{item.badge}</span>
                  ) : null}
                </div>
                <p className="text-sm text-muted-foreground">{item.meta}</p>
                {item.submeta ? <p className="mt-1 text-xs text-muted-foreground">{item.submeta}</p> : null}
              </div>
              <div className="ml-4 flex gap-2">
                {item.onEdit ? <Button variant="ghost" size="sm" onClick={item.onEdit}><Edit2 className="h-4 w-4" /></Button> : null}
                {item.onDelete ? <Button variant="ghost" size="sm" onClick={item.onDelete} className="text-destructive"><Trash2 className="h-4 w-4" /></Button> : null}
              </div>
            </Card>
          ))}
        </div>
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-40 border-b border-border bg-card/80 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-4 sm:px-6 lg:px-8">
          ...
          <Button
            onClick={async () => { await api.logout(); navigate("/admin/login"); }}
            variant="outline"
            size="sm"
          >
            Logout
